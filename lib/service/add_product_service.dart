...
class AddProductService {
  ...
  Future<int> addProduct(AddProductModel product, List<XFile?> images) async {
    ...
    print(ProfileController.to.getId());
    print(response.body);
    if (response.statusCode == 200) {
      ...
      throw Exception('addProductService 오류');
    }
  }
  ...
  Future<String> uploadImage(List<XFile?> images, int productId) async {
    ...
    final response = await Dio().post('$imageUrl$productId/images/upload',
        data: formData, options: Options(contentType: 'multipart/form-data'));

    if (response.statusCode == 200) {
      return "이미지 업로드됨";
    } else {
      throw Exception('addProductService 이미지 업로드 오류');
    }
  }
  ...
  Future<String> uploadProfileImage(List<XFile?> images, int userId) async {
    final List<MultipartFile> files = images
        ...
        .toList();
    final formData = FormData.fromMap({'file': files});
    final response = await Dio().post('$profileUrl$userId/images/upload',
        data: formData,
        options: Options(
          contentType: 'multipart/form-data',
          headers: {
            'Cookie': ProfileController.to
                .getCookies('$profileUrl$userId/images/upload')
          },
        ));
